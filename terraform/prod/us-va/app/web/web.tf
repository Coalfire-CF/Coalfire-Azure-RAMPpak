resource "azurerm_availability_set" "web_availability_set" {
  name                         = "${local.resource_prefix}-web1-as"
...
module "web1" {
  ...
  vm_name                       = "${local.vm_name_prefix}web1"
  ...
  subnet_id                     = data.terraform_remote_state.use2_app_vnet.outputs.use2_app_vnet_subnet_ids[1]
...
module "web2" {
  ...
  vm_name                       = "${local.vm_name_prefix}web2"
  vm_admin_username             = var.vm_admin_username
  location                      = var.location
  resource_group_name           = data.terraform_remote_state.setup.outputs.application_rg_name
  size                          = "Standard_D4s_v3"
...
module "web-nsg" {
  ...
  security_group_name         = "${local.resource_prefix}-web-nsg"
  ...
  custom_rules = [
    ...
    },
    {
      ...
      source_address_prefixes = [data.terraform_remote_state.use2_app_gateway.outputs.use2_app_gateway_ip, "${var.ip_network_app}.2.0/24"]
      description             = "HTTPS"
    }
  ]
}
...
module "web-nsg-diag" {
  ...
  resource_id           = module.web-nsg.network_security_group_id
  diag_eventhub_name    = data.terraform_remote_state.setup.outputs.diag_eventhub_name
  diag_eventhub_auth_id = data.terraform_remote_state.setup.outputs.diag_eventhub_auth_id
...
resource "azurerm_availability_set" "web_availability_set_2" {
  name                         = "${local.resource_prefix}-web2-as"
  location                     = var.location
  resource_group_name          = data.terraform_remote_state.setup.outputs.application_rg_name
  platform_update_domain_count = 2
  platform_fault_domain_count  = 2
...
module "web3" {
  ...
  vm_name                       = "${local.vm_name_prefix}web3"
  vm_admin_username             = var.vm_admin_username
  location                      = var.location
  resource_group_name           = data.terraform_remote_state.setup.outputs.application_rg_name
  size                          = "Standard_D4s_v3"
...
module "web4" {
  ...
  vm_name                       = "${local.vm_name_prefix}web4"
  vm_admin_username             = var.vm_admin_username
  location                      = var.location
  resource_group_name           = data.terraform_remote_state.setup.outputs.application_rg_name
  size                          = "Standard_D4s_v3"
