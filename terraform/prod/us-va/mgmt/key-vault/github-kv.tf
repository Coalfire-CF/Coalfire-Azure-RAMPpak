module "github_kv" {
  ...
  kv_name                         = "${local.resource_prefix}-gh-kv"
...
# Create SSH keys for github and store in AKV
resource "tls_private_key" "githubkey" {
  algorithm = "RSA"
}

# resource "azurerm_key_vault_secret" "github_ssh" {
#   name         = "gh-ssh-private-key"
#   value        = base64encode(tls_private_key.githubkey.private_key_openssh)
#   key_vault_id = module.github_kv.key_vault_id
